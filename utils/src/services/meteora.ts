...
export class MeteoraService {
     ...
     */
    parsePositionOpenEvents(transactions: TransactionDetails[]): PositionOpenEvent[] {
        ...
        for (const tx of transactions) {
            try {
                ...
                for (const instruction of meteoraInstructions) {
                    ...
                    if (this.isAddLiquidityInstruction(instruction, tx.meta.logMessages)) {
     ...
     */
    private isAddLiquidityInstruction(instruction: any, logMessages: string[]): boolean {
     ...
     */
    async fetchPosition(positionPubkey: PublicKey, lbPair: PublicKey): Promise<LbPosition | null> {
        try {
            ...
            console.log("🚀 ~ MeteoraService ~ fetchPosition ~ positionInfo:", positionInfo.positionData)
     ...
     */
    private deserializePosition(pubkey: PublicKey, accountInfo: AccountInfo<Buffer>): Position {
        const data = accountInfo.data;
        console.log("🚀 ~ MeteoraService ~ deserializePosition ~ data:", data)
        if (data.length < 8) {
            throw new Error(`Invalid position account data: too small (${data.length} bytes)`);
        }
        ...
        if (data.length < offset + 64) { // Need at least 64 bytes for lbPair + owner
            throw new Error(`Invalid position account data: insufficient data for basic fields`);
        }
        ...
        if (data.length < offset + 4) {
            console.warn(`Position ${pubkey.toBase58()}: No bin data available, creating empty position`);
            return {
                pubkey,
                lbPair,
                owner,
        ...
        if (binCount > METEORA_CONSTANTS.MAX_BIN_PER_POSITION) {
            throw new Error(`Invalid bin count: ${binCount} exceeds maximum ${METEORA_CONSTANTS.MAX_BIN_PER_POSITION}`);
        }
        const binIds: number[] = [];
        ...
        if (data.length < offset + requiredBytesForBins) {
            console.warn(`Position ${pubkey.toBase58()}: Insufficient data for ${binCount} bins, reading available bins only`);
     ...
     */
    private deserializeLbPair(pubkey: PublicKey, accountInfo: AccountInfo<Buffer>): LbPair {
        const data = accountInfo.data;
        let offset = 8; // Skip discriminator
    ...
    calculateBinPrice(bins: PositionBinData[]): BinPriceResult | null {
        ...
        const calculatePriceStep = (): {step: number, stepPerToken:number} => {
            ...
            const stepPerToken = parseFloat(bins[1].pricePerToken) - parseFloat(bins[0].pricePerToken);
            return {step, stepPerToken};
        };

        // First, try to find active bin
        const activeBin = bins.find(bin =>
            bin.positionXAmount !== '0' && bin.positionYAmount !== '0'
        );
        ...
        if (firstBin.positionYAmount === '0') {
            ...
            estimatedPricePerToken = parseFloat(firstBin.pricePerToken) - priceStep.stepPerToken;
        ...
        // If last bin has no X amount, price is above last bin  
            ...
            estimatedPricePerToken = parseFloat(lastBin.pricePerToken) + priceStep.stepPerToken;
     ...
     */
    getPriceRange(bins: PositionBinData[]): { minPrice: number; maxPrice: number } {
     ...
     */
    async fetchBins(lbPairPubkey: PublicKey, binIds: number[]): Promise<Map<number, Bin>> {
        ...
        try {
            ...
            const connection = await this.quickNodeService.getConnection();
            const accountInfos = await this.quickNodeService.getMultipleAccountsInfo(binArrayAddresses);
     ...
     */
    private getBinArrayAddresses(lbPairPubkey: PublicKey, binIds: number[]): PublicKey[] {
        const binArrayIndexes = new Set<number>();

        for (const binId of binIds) {
            const binArrayIndex = Math.floor(binId / METEORA_CONSTANTS.MAX_BIN_PER_ARRAY);
     ...
     */
    private deriveBinArrayAddress(lbPairPubkey: PublicKey, index: number): PublicKey {
     ...
     */
    private deserializeBinArray(accountInfo: AccountInfo<Buffer>): Map<number, Bin> {
        const bins = new Map<number, Bin>();
        const data = accountInfo.data;
        ...
        for (let i = 0; i < METEORA_CONSTANTS.MAX_BIN_PER_ARRAY; i++) {
            const binId = Number(binArrayIndex) * METEORA_CONSTANTS.MAX_BIN_PER_ARRAY + i;
            ...
            if (!reserveX.isZero() || !reserveY.isZero() || !liquiditySupply.isZero()) {
                bins.set(binId, {
                    binId,
    ...
    calculateUnCollectedFees(
        ...
        for (let i = 0; i < position.binIds.length; i++) {
            ...
            if (bin && !liquidityShare.isZero()) {
                ...
                const userFeeX = bin.feeInfos.baseFeeRateX.mul(shareRatio).div(new BN(1000000));
                const userFeeY = bin.feeInfos.baseFeeRateY.mul(shareRatio).div(new BN(1000000));
    ...
    async getTokenDecimals(mintAddress: PublicKey): Promise<number> {
        try {
            const tokenSupply = await this.quickNodeService.getTokenSupply(mintAddress);
            return tokenSupply.decimals;
        } catch (error) {
            console.error(`Error getting token decimals for ${mintAddress.toBase58()}:`, error);
