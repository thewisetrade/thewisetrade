  ...
  switch (strategy) {
    ...
    case 'spot':
      ...
      for (const binId of binIds) {
        ...
        const liquidity = totalLiquidity.mul(new BN(weight)).div(new BN(binIds.reduce((sum, id) => {
    ...
    case 'curve':
      ...
      for (const weight of weights) {
        const liquidity = totalLiquidity.mul(new BN(Math.floor(weight * 1000000))).div(new BN(Math.floor(totalWeight * 1000000)));
        distribution.push(liquidity);
      }
      break;
  }
...
export function calculateVWAP(
  ...
  return totalVolume.isZero() ? new Decimal(0) : totalVolumePrice.div(totalVolume);
}
...
export function calculateSharpeRatio(
  ...
  const meanReturn = returns.reduce((sum, ret) => sum.plus(ret), new Decimal(0)).div(returns.length);
...
export function calculateCorrelation(
  ...
  const mean1 = series1.reduce((sum, val) => sum.plus(val), new Decimal(0)).div(n);
  const mean2 = series2.reduce((sum, val) => sum.plus(val), new Decimal(0)).div(n);
...
export function calculateCompoundInterest(
  ...
  return principal.mul(
    new Decimal(1).plus(ratePerPeriod).pow(totalPeriods)
  );
}
